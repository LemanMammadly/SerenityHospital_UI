...
const Index = () => {
    ...
    useEffect(() => {
        axios
          .get(
            `https://localhost:7227/api/PatientHistories`, {
              headers: {
    ...
    const seacrhChange = (key) => {
      ...
      item && item.recipe && item.recipe.doctor &&  item.recipe.doctor.name &&  item.recipe.doctor.name.toLowerCase().includes(key.toLowerCase())
      );
